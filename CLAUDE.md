...
- Muestra chica y variable: la cantidad de puntos depende de cuánta historia real tenga cada ticker -- con las ~350 barras que carga `REFRESH_DAYS=500` alcanza para horizontes largos (ver "Frontend" para el detalle de por qué 500 y no menos), pero un ticker con menos historia real igual va a dar pocos puntos. Los resultados con pocas observaciones por bucket no son estadísticamente concluyentes, son una señal direccional para justificar la ponderación del score, no un backtest de nivel institucional.
...
- `POST /api/watchlist/<int:pk>/refresh/` -- para cada `Ticker` del watchlist corre `load_prices_for_ticker` (`REFRESH_DAYS`, ver "Frontend" para el valor y el porqué) y después `compute_score`; devuelve un resumen JSON (`tickers_procesados`, `errores_de_precio`, `scores_actualizados`, `omitidos_por_falta_de_historico`) -- mismo espíritu que el resumen de los management commands, como response en vez de stdout. Un ticker roto no tumba el resto. 404 si el watchlist no existe.
...
**`REFRESH_DAYS = 500`** (días calendario, ~350 barras de trading) -- duplicado en `dashboard/views.py` y `api/views.py::WatchlistRefreshView`, sincronizados en valor a propósito (ver arriba). **Bug real, encontrado contra datos reales, no con mocks:** con el valor anterior (`90`, ~80-90 barras) dos cosas fallaban en simultáneo -- (1) el chart de precio necesita `CHART_DISPLAY_BARS` (60) barras visibles + 50 de warm-up previo para que la SMA50 no aparezca `None`/cortada en los primeros ~2 meses del rango visible (110 barras mínimo), y (2) el backtest con `horizon_days` > ~30 sesiones no tenía suficientes barras futuras para generar un solo punto (`run_backtest` necesita `MIN_BARS_REQUIRED` (50) + `horizon_days` barras como mínimo). 500 días da margen cómodo para ambos casos. `CHART_HISTORY_BARS` (200) y `CHART_DISPLAY_BARS` (60) no cambiaron -- con 350 barras disponibles, 200 de historia para el chart ya alcanza de sobra.

**`market_data.services.load_prices_for_ticker` -- fetch incremental, no `REFRESH_DAYS` completos en cada refresh.** Segundo bug real de producción (logs de Render: "WORKER TIMEOUT" + `SIGKILL` por presunto out-of-memory -- **no** una excepción de Python, así que ningún mock lo reproduce). Subir `REFRESH_DAYS` a 500 arregló SMA50/backtest pero volvía a bajar los 500 días completos de yfinance en **cada** refresh, para **cada** ticker del watchlist, secuencial dentro del mismo request -- con varios tickers de historia profunda (AAPL/DE/GOOGL/MSFT/NVDA) la cadena completa no entraba en el timeout del worker de Gunicorn del plan free/starter. Fix: `days` completos son ahora **solo el bootstrap único** de un ticker sin ningún `PriceBar` todavía; si el ticker ya tiene historial, el refresh pide a yfinance desde `(último date guardado - OVERLAP_DAYS)` (`OVERLAP_DAYS = 5`, margen para capturar revisiones tardías de datos), nunca `days` de nuevo. **Limitación conocida, no resuelta:** si se agregan varios tickers *nuevos* al mismo watchlist y se aprieta "Refrescar precios" una sola vez, todavía se hace el bootstrap completo de cada uno en el mismo request -- agregar tickers de a uno y refrescar es el flujo normal que evita el timeout.
