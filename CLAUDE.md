...
- `GET /dashboard/` (`DashboardView.get`) -- lista los `Watchlist` con sus tickers y el `Score` más reciente de cada uno (mismo `prefetch_related("items__ticker__scores")` que `api/views.py::WatchlistListView`, sin N+1). `POST /dashboard/` (`DashboardView.post`, botón "Refrescar precios") corre el mismo loop que `api/views.py::WatchlistRefreshView` -- `load_prices_for_ticker` + `compute_score` por ticker -- pero invocando los servicios directo, no la vista DRF; el resumen se muestra como mensaje flash (`django.contrib.messages`), no como JSON.
- `GET /dashboard/ticker/<symbol>/` (`TickerDetailView`) -- Score vigente con su `components`, chart de precio+SMA20+SMA50 (Chart.js, últimos 60 `PriceBar`; la SMA se recalcula día a día llamando `scoring.indicators.sma` repetidamente, mismo patrón que usa `backtesting/services.py` para el score), y la sección "Agente explicador". Sin `Score` calculado: estado vacío explicando que hace falta `compute_scores`, nunca un error crudo.
